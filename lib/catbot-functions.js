...
module.exports = {
	...
	reply: function(catbot, slackbot, message) {
		...
		for (var i in aMsg) {
			...
			if ((word == "gif") || (word == "gifs") || (word == "giphy")) {
				silent = true
				send_to_slack = ""
				this.catGif(slackbot, message)
			}
			if ((word == "sticker") || (word == "stickers")) {
				silent = true
				send_to_slack = ""
				this.catGif(slackbot, message, "sticker")
			}
		}
  ...
  randomPlaying: function() {
	...
	catGif: function(bot, message, sticker = false) {
		var msg = message
		if (message.text) {
			msg = message.text
		}
	    ...
	    if (err) {
	      ...
	      for (var data in received) {
	        ...
	        gif = giphy.url //embed_url
					...
					console.log(giphy.original_mp4)
	        if (gif) {

	          console.log("@catbot: " + gif)
