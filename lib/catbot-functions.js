...
module.exports = {
	...
	language_detect: function(msg) {
		...
		console.log("Language guess: " + lang)
		return lang
	},
	i18n: function(lang) {
...
//	global_meow.tl = "Ngiyaw" // just no
  ...
  // * Support functions
	...
	parseCatCommands: function(cb_input) {
		var output = []

		// Manual overrides (catbot commands):
		if (cb_input.includes("purr"))      { output.push("Purrr") }
		if (cb_input.includes("moo"))       { output.push("Moooo") }
		if (cb_input.includes("oink"))      { output.push("🐽") }
		if (cb_input.includes("cluck"))     { output.push("Bwaaak!") }

		if (cb_input.includes("quack"))     { output.push("Quack!") }
		if (cb_input.includes("duck"))     { output.push("Quack!") }
		if (cb_input.includes("fuck"))     { output.push("Quack!") }
